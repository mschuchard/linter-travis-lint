  ...
  provideLinter() {
    return {
      ...
      lint: (activeEditor) => {
        ...
        const regex_travis = /\[x\] \[(\w+)\] (.*)/;
        const regex_yaml = /STDIN is not valid YAML: (.*) at line (\d+) column (\d+)/;
        ...
        return helpers.exec(atom.config.get('linter-travis-lint.travisExecutablePath'), args, {stdin: content}).then(output => {
          output.split(/\r?\n/).forEach((line) => {
            ...
            if (matches_travis != null) {
              toReturn.push({
                severity: matches_travis[1] == 'error' ? 'error' : 'warning',
                excerpt: matches_travis[2],
                location: {
                  file: file,
                  position: [[0, 0], [0, 1]],
                },
        ...
        .catch(error => {
          ...
          const matches_yaml = regex_yaml.exec(error);
